...
/**
 * Class LWP_Infobip_Api class.
 */
class LWP_Infobip_Api {
	...
	public function __construct() {
        ...
        $this->user=$options['idehweb_infobip_account_sid'];
        $this->password=$options['idehweb_infobip_password'];
        $this->sender=$options['idehweb_infobip_sender'];
//        $this->api_key   = trim( $plugin_settings['api_key'] );
	}

	public function lwp_send_sms( $phone, $text ) {

		$url      = 'https://api.infobip.com/api/v3/sendsms/plain?user='.$this->user.'&password='.$this->password.'&sender='.$this->sender.'&SMSText='.$text.'&GSM='.$phone.'';
